...
const RulesForm: React.FC<RulesFormProps> = ({ setGameRules, playerCount }) => {
  ...
  const [undercoverCount, setUndercoverCount] = useState<string>(
    String(initialUndercoverCount)
  );
  const [hasMrWhite, setHasMrWhite] = useState<boolean>(false);

  const clamp = (value: number) =>
    Math.max(0, Math.min(playerCount - 1, value));

  const handleUndercoverCountChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setUndercoverCount(event.target.value);
  };

  const handleUndercoverCountBlur = () => {
    const value = Number(undercoverCount);
    const clamped = clamp(Number.isNaN(value) ? 0 : value);
    setUndercoverCount(String(clamped));
  };
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    setGameRules(clamped, hasMrWhite);
  };

  return (
    <form
      onSubmit={handleSubmit}
      className="flex flex-col items-center w-full sm:w-3/4 md:w-1/2 lg:w-1/3 space-y-10 p-4 sm:p-6 md:p-8 bg-white border border-gray-200 rounded-lg shadow"
    >
      <div className="max-w-sm p-8 bg-white border border-gray-200 rounded-lg shadow space-y-3 flex flex-col items-center">
        <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 font-mono">
          Espiões
        </h5>
        ...
        <div className="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset w-full">
          <input
            ...
            onBlur={handleUndercoverCountBlur}
      ...
      <div className="max-w-sm p-8 bg-white border border-gray-200 rounded-lg shadow space-y-2 flex flex-col items-center">
        ...
        <div className="flex items-center">
          <input
            ...
            className="mr-2"
          />
          <label className="font-mono">Jogar com Mr. White</label>
        </div>
      </div>
